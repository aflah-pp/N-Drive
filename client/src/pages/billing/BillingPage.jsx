...
function BillingPage() {
  ...
  const handleBuy = async (plan) => {
    try {
      ...
      if (response.data?.result.order_id) {
        navigate(
          `/payment?order_id=${response.data.result.order_id}&amount=${response.data.result.amount}`,
        );
      } else {
      ...
      <div className="max-w-screen-xl mx-auto ">
        <div className="text-center mb-12">
          ...
          <p className="mb-5 text-[#5a4e3d] sm:text-lg">
            Pick the right balance of features and value — start small or go all in.
          </p>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex justify-center items-baseline my-6">
                  <span className="mr-2 text-5xl font-extrabold text-[#99744a]">₹{plan.price}</span>
                </div>
...
function Feature({ text }) {
  return (
    <li className="flex items-center space-x-3 text-[#414a37]">
      <svg className="flex-shrink-0 w-5 h-5 text-[#99744a]" fill="currentColor" viewBox="0 0 20 20">
        <path
          fillRule="evenodd"
